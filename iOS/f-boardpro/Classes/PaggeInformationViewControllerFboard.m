...
@interface PaggeInformationViewControllerFboard ()

@end

@implementation PaggeInformationViewControllerFboard
...
-(void)createFollowTable
{
    ...
    feedTableView=[[UITableView alloc]initWithFrame:CGRectMake(10, 50, self.view.frame.size.width-20,self.view.frame.size.height+10) style:UITableViewStylePlain];
    ...
    if (token)
    {
        ...
        
        {
            ...
            for (NSDictionary *dic in [result objectForKey:@"data"] )
            {
                ...
                pageId= [[dic objectForKey:@"from"]objectForKey:@"id"];
                ...
                if (pageId) {
                    [self.fromIdArray addObject:pageId];
                }else{
...
-(void)getBannertUrl:(NSString *)pageId
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSURL * url=[NSURL  URLWithString:[NSString stringWithFormat:@"https://graph.facebook.com/%@?fields=cover&access_token=%@",pageId,[SingletonClass sharedState].accessTokenString]];
